...
* If your node has multiple network interfaces, kubelet may select an interface with an IP address outside of your remote node network CIDRs. To use a different interface, specify its IP address using the `--node-ip` kubelet flag in your `nodeConfig.yaml`. For more information, see <<hybrid-nodes-nodeadm>>.
